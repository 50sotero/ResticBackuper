...
export default function LoadingState({
  label,
  variant = "Drive",
  // Desktop integration never fetches a demo clip from an external host.
  videoSrc = "",
}: LoadingStateProps) {
  const elapsed = useElapsed();
  const surfer = variant === "Surfer" && !!videoSrc;
  ...
  const elapsedEl = <span aria-hidden="true" className="font-mono text-[12px] text-ink-3 tabular-nums">{elapsed}</span>;
